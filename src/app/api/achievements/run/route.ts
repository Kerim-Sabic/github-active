...
async function runPullRequestFlow(input: {
  ...
  for (let i = 1; i <= count; i += 1) {
    ...
    const branch = await createBranchWithRetry({
      ...
      sandbox,
      baseSha,
      buildName: () => `${kind === "yolo" ? "yolo" : "feature"}/${entry.path.split("/").slice(-1)[0]?.replace(/\.[^.]+$/, "") ?? `${prefix}-${ts}-${i}`}-${randomSuffix()}`,
      send
    });

    send("step", { phase: "commit", message: `commit ${entry.path}` });
    await putFile({
      token,
      owner: sandbox.owner,
      repo: sandbox.repo,
...
async function runPairExtraordinaire(input: {
  ...
  const branch = await createBranchWithRetry({
    ...
    sandbox,
    baseSha,
    buildName: () => `pair/${entry.path.split("/").slice(-1)[0]?.replace(/\.[^.]+$/, "") ?? `pair-${ts}`}-${randomSuffix()}`,
    send
  });
